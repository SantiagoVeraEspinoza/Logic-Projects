...
public:
	...
	void ResolveColision(int tarID=1) {
		float Dif = 0.3;
		if (R[0].vel.x > 0 and RectVSRect(R[0].pos.x, ((LastPos.y) + (Dif)), R[0].size.x, ((R[0].size.y) - (Dif * 2) - 1),
				R[tarID].pos.x, R[tarID].pos.y, R[tarID].size.x, R[tarID].size.y - 0.3)) 
			R[0].pos.x = R[tarID].pos.x - R[0].size.x;
		else if (R[0].vel.x < 0 and RectVSRect(R[0].pos.x, ((LastPos.y) + (Dif)), R[0].size.x, ((R[0].size.y) - (Dif * 2) - 1),
				R[tarID].pos.x+1, R[tarID].pos.y, R[tarID].size.x-1, R[tarID].size.y - 0.3))
			R[0].pos.x = R[tarID].pos.x + R[tarID].size.x;

		else if (R[0].vel.y > 0 and RectVSRect(LastPos.x + 0.2, R[0].pos.y, R[0].size.x - 1.4, R[0].size.y,
				R[tarID].pos.x, R[tarID].pos.y, R[tarID].size.x, R[tarID].size.y-1)) 
			R[0].pos.y = R[tarID].pos.y - R[0].size.y;
		else if (R[0].vel.y < 0 and RectVSRect(LastPos.x + 0.2, R[0].pos.y, R[0].size.x - 1.4, R[0].size.y,
				R[tarID].pos.x, R[tarID].pos.y+1, R[tarID].size.x, R[tarID].size.y-1))
